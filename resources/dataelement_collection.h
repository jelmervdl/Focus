#ifndef __DATAELEMENT_COLLECTION__
#define __DATAELEMENT_COLLECTION__

#include "collection.h"

class DataElementCollection : public Collection
{   
    
 public:
    DataElementCollection(std::string name, std::string type, std::string dir_name)
    :
    ...
    void load()
    {
        ...
        DataElementConverter element_converter(context);

        log("before fast get ids", EXTRA);
        FastAllocator::id_vector ids(fast_allocator.get_ids());
        log(ids.size(), EXTRA);
        
        log2("reading collection, size: ", ids.size(), EXTRA);
