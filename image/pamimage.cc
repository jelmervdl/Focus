...
PamImage::PamImage(std::vector<float> const &v, uint32_t in_width, uint32_t in_height)
{
    assert(in_width * in_height == v.size());
    ...
    std::vector<float>::const_iterator it(v.begin()), it_end(v.end());
    
    float mean = vector_mean(v);
    float sd = vector_norm(vector_difference(v, mean));
    float min_val = mean - 1.0 * sd;
    float max_minus_min = 2.0 * sd;
    
    for (uint32_t row(0); row < in_height; ++row)
        for (uint32_t column(0); column < in_width; ++column, ++it)
            ppGrayPixels[row][column] = static_cast<GrayPixel>(std::max<float>(0.0, std::min<float>(GRAY_MAX, (*it - min_val) / max_minus_min * GRAY_MAX)));
}
...
PamImage::PamImage(std::istream &in_stream)
{
    iImageType = NO_IMAGE;
    PamImage::loadImage(in_stream);
    if (in_stream)
        sFileName = byte_read<string>(in_stream);
}
...
void PamImage::resizeGray(long w, long h)
{
  _createGrayPixels(w, h);
}
...
// Load pixels from grayscale image file (P2).
void PamImage::_loadGrayPixelsPlain(std::istream &in_stream)
{
    PamImage::_createGrayPixels(width, height);
    for (long y = 0; y < height; y++) {
        for (long x(0); x < width; ++x)
            in_stream >> ppGrayPixels[y][x];
    }
}

void PamImage::_loadGrayPixels(std::istream &in_stream)
{
    PamImage::_createGrayPixels(width, height);
    for (long y = 0; y < height; y++) {
        for (long x(0); x < width; ++x)
          ppGrayPixels[y][x] = byte_read<GrayPixel>(in_stream);
    }
}
...
// load .pbm (P4), .pgm (P5) or .ppm (P6)
void PamImage::loadImage(std::istream &in_stream)
{
    char magicP;  // The leading 'P' in the file
    int type;     // 4, 5, or 6; defines image type
    int gmax = 0; // Maximum gray value; not used any further.
    ...
    {
        iImageType = BW_IMAGE;
        _loadBWPixels(in_stream);
    }
    else if (type == 5) // P5
    {
        iImageType = GRAY_IMAGE;
        _loadGrayPixels(in_stream);
    }
    else if (type == 6) // P6
    {
        iImageType = RGB_IMAGE;
        _loadRGBPixels(in_stream);
    }
    else if (type == 2)
      {
        iImageType = GRAY_IMAGE;
        _loadGrayPixelsPlain(in_stream);
      }
...
std::vector<float> PamImage::to_vector()
{
    assert(getImageType() == 2);
    int width(getWidth());
    int height(getHeight());
    std::vector<float> the_vector(width * height);
    
    GrayPixel **array = getGrayPixels();
    std::vector<float>::iterator it = the_vector.begin();
    
    for (int y(0); y < height; ++y)
        for (int x(0); x < width; ++x, ++it)
            *it = (static_cast<float>(array[y][x]) / 255.) * (picture_to_float_max - picture_to_float_min) + picture_to_float_min;
    return the_vector;
}

void PamImage::load_from_binary_stream(std::istream &in_stream)
{
    //header: imagetype, width, height
    //data: pixels
    iImageType = byte_read<int>(in_stream);
    if (iImageType != GRAY_IMAGE)
        throw NotSupportedError();
    sFileName = byte_read<string>(in_stream);

    width = byte_read<long>(in_stream);
    height = byte_read<long>(in_stream);
    resizeGray(width, height);
    log("going to read pixels " + t_to_string<>(width) + " " + t_to_string<>(height));
...
void PamImage::save_to_binary_stream(std::ostream &out_stream)
{
    byte_write<>(out_stream, iImageType);
    byte_write<>(out_stream, sFileName);
    byte_write<>(out_stream, width);
    byte_write<>(out_stream, height);

    for (int x(0); x < width; ++x)
        for (int y(0); y < height; ++y)
        {
            log(t_to_string<>(x) + " " + t_to_string<>(y));
            byte_write<>(out_stream, ppGrayPixels[y][x]);
        }
}
